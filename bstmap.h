...
private:
  ...
  void copy_helper(Node *original, Node *other);
  Node *createBalancedTree(const vector<value_type> &v, int start, int end);
  void inorderHelper(const Node *curr,
                     std::function<void(const value_type &)> visit) const;
  int countNodes(const Node *curr) const;
  void preorderHelper(const Node *curr, void visit(const value_type &)) const;
  void postorderHelper(const Node *curr, void visit(const value_type &)) const;
